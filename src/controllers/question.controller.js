...
export const getAllQuestionByTest = async (req, res) => {
  try {
    ...
    return success(res, "Get all question by test success", {
...
export const getAllQuestionByPart = async (req, res) => {
  try {
    ...
    return success(res, "Get all question by part success", {
      data: {
        parts,
        questions: {
          items: questions,
...
export const createQuestions = async (req, res) => {
  ...
  try {
    ...
    const lastQuestion = await Question.findOne({ partId }).sort({
      questionNumber: -1,
    })
    let questionNumber = lastQuestion ? lastQuestion.questionNumber : 0
    let globalQuestionNumber = await Question.countDocuments({
      testId: test._id,
    })
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (!q.choices || q.choices.length !== expectedChoices) {
        return error(
          res,
          `Part ${part.partNumber} requires ${expectedChoices} choices. (Question ${i + 1})`,
        )
      }
    ...
    await Part.findByIdAndUpdate(partId, {
      $inc: { totalQuestion: inserted.length },
    })
...
export const updateQuestion = async (req, res) => {
  try {
    ...
    // lấy part để biết partNumber
    const part = await Part.findById(q.partId)
    ...
    const validAnswers =
      part.partNumber === 2 ? ["A", "B", "C"] : ["A", "B", "C", "D"]

    // question có thể rỗng
    ...
    // update choices
    if (choices) {
      const expectedLength = part.partNumber === 2 ? 3 : 4

      if (!Array.isArray(choices) || choices.length !== expectedLength) {
        return error(res, `Part ${part.partNumber} requires ${expectedLength} choices`, 400)
      }
    ...
    if (correctAnswer) {
      if (!validAnswers.includes(correctAnswer)) {
        return error(res, `Correct answer must be ${validAnswers.join(", ")}`, 400)
      }
    ...
    return success(res, "Question updated successfully", q)
  } catch (err) {
